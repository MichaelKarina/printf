 ...
 * @size: Size specifier
 *
 * Return: Number of charc printer.
 */
int handle_write_char(char c, char buffer[],
	int flags, int width, int precision, int size)
{ /* char is stored at left and paddint at buffer's right */
	int i = 0;
	char padd = ' ';
 ...
 * @size: Size specifier
 *
 * Return: Numbersof chars printers
 */
 ...
 * @extra_c: Extra char
 *
 * Return: Number of printed charct
 */
	...
	int length, char padd, char extra_c)
{
	...
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0')
		buffer[ind] = padd = ' '; /* width is displayes with padding ' ' */
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ')/* Assignextra char to left of buffer */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
 ...
 * @size: Size specifier
 *
 * Return: Number of written charct
 */
int write_unsgnd(int is_negative, int ind,
	char buffer[],
	int flags, int width, int precision, int size)
{
	/* The numbersis stored at the bufer's right and starts at position i */
 ...
 * @padd: chac representing the padding
 * @extra_c: Chac representing extra chac
